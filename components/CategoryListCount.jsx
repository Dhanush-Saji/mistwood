import { getCategoryCount } from '@/actions/server-action'
...
const CategoryListCount = async({data,search}) => {
    console.log(search)
    const categoryTab = await getCategoryCount()
  ...
  return (
    <>
      <div className="hidden md:grid grid-cols-5 gap-[1.5rem] w-full justify-center mt-[0.7rem]">
        {
          categoryTab?.length>0 && categoryTab?.map((category,index)=>(
            <Link scroll={false} href={`/shop?category=${category.name}`} className={`${((data == category.name) || (category.name == 'All' && data == null))?'bg-[#ffde3c] border-[#e6ca43]':'border-[rgba(0,0,0,0.2)] hover:border-slate-400'} border  flex rounded-[8px] p-2 gap-3 ease-in transition-all duration-200 hover:scale-105  hover:shadow-md`} key={index}>
              <div className={`${((data == category.name) || (category.name == 'All' && data == null))?'bg-[#cfc532]':'bg-[#ECECED]'} border border-[rgba(0,0,0,0.2)] rounded-[4px] flex items-center justify-center p-2 `}>
      ...
      {
          categoryTab?.length>0 && categoryTab?.map((category,index)=>(
            <Link scroll={false} href={`/shop?category=${category.name}`} className={`${((data == category.name) || (category.name == 'All' && data == null))?'bg-[#ffde3c] border-[#e6ca43]':'border-[rgba(0,0,0,0.2)] hover:border-slate-400'} border  flex rounded-full px-[0.5rem] py-[0.2rem]  md:p-2 gap-3 ease-in transition-all duration-200 hover:scale-105  hover:shadow-md justify-center items-center`} key={index}>
              <p className="font-medium">{category?.name}</p>
              </Link>
          ))
        }
