...
const CartShowBtn = () => {
    ...
    return (
        <Link href={'/cart'} prefetch={false} onClick={popUpMessage}>
            <div className='rounded-full text-[#27282a] dark:text-white p-1.5 relative flex items-center justify-center'><ShoppingCart />
                <span className='w-5 h-5 flex items-center dark:text-[#27282a] justify-center bg-[#ffde3c] absolute right-[-5px] top-[-5px] text-xs font-bold rounded-full'>{cartQnty}</span>
            </div>
        </Link>
    )
}
